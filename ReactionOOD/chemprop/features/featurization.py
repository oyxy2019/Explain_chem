        ...
        if not self.is_reaction:
            ...
            for a1 in range(self.n_atoms):
                for a2 in range(a1 + 1, self.n_atoms):
                    ...
                    # Update index mappings
                    self.edge_list.append((a1,a2))
                    self.edge_features.append(self.f_atoms[a1] + f_bond)
                    self.edge_list.append((a2,a1))
                    self.edge_features.append(self.f_atoms[a2] + f_bond)
                    b1 = self.n_bonds
                    b2 = b1 + 1
                    self.a2b[a2].append(b1)  # b1 = a1 --> a2
                    self.b2a.append(a1)
    ...
    * :code:`b2br`: (Optional): A mapping from f_bonds to real bonds in molecule recorded in targets.
    ...
    def __init__(self, mol_graphs: List[MolGraph]):
        ...
        self.bond_fdim = get_bond_fdim(overwrite_default_bond=self.overwrite_default_bond_features,
                                      ...
                                      is_reaction=self.is_reaction)
        # print(self.atom_fdim,self.bond_fdim)
        # exit()
