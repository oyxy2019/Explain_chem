...
def make_mol(s: str, keep_h=False, add_h=False, keep_atom_map=False):
    ...
    mol = Chem.MolFromSmiles(s, params)     # 有的data会报警告WARNING: not removing hydrogen atom without neighbors
...
class GOODE2SN2(InMemoryDataset):
    r"""
    The GOOD-E2SN2 dataset. Adapted from 
    `Thousands of reactants and transition states for competing e2 and s2 reactions`_.
    ...
    def get_domain_sorted_list(self, data_list, domain='scaffold'):
        ...
        for data in sorted_data_list:
            if getattr(data, domain) != cur_domain:
                ...
                cur_domain_id += 1
                # print(f"cur_domain_id={cur_domain_id}")
    ...
    def process(self):
        ...
        for i, final_data_list in enumerate(all_data_list):
            print(f'index {i}: {self.processed_paths[i]} length: {len(final_data_list)}')

        for i, final_data_list in enumerate(all_data_list):
            if not final_data_list:
                print(f"final_data_list at index {i} is empty.")
                continue
...
if __name__ == '__main__':
    dataset, meta_info = GOODE2SN2.load(dataset_root='ReactionOOD', domain='size', shift='concept', generate=True)
    """
        Args:
            root (str): The dataset saving root.
            domain (str): The domain selection. Allowed: 'scaffold' and 'size'.
