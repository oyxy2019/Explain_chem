...
from .. import ExplainerConfig, Explanation, ModelConfig
from ..algorithm import ExplainerAlgorithm
from ..algorithm.utils import clear_masks, set_masks
from ..config import MaskType, ModelMode, ModelTaskLevel
    ...
    def _train(
        ...
        **kwargs,
        ...
        for i in range(self.epochs):
            ...
            # y_hat, y = model(h, edge_index, **kwargs), target     # 这里因为model接口不一致，所以h和edge_index没有被传参过去
            data = kwargs.get('data')
            batch_size = kwargs.get('batch_size')
            data.x = h
            data.edge_index = edge_index
            y_hat, y = model(data=data, batch_size=batch_size), target
            ...
            # 在第一次迭代(i==0)中，我们收集参与预测的节点和边。这些是梯度!= 0(未应用正则化)的所有节点和边。
