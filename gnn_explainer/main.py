...
print(torch.cuda.get_device_name(device) if torch.cuda.is_available() else "CPU")
...
for i, data in enumerate(dataset):
    if data.mol.split(">")[0] == "[Cl-:2].[Cl:1][C@:5]([C:4]([H:3])([H:31])[N:41]([H:42])[H:43])([C:11]#[N:12])[N:21]([H:22])[H:23]":
        ...
        model, _ = load_good_model_dataset(config_path, device)
        data0 = get_pygdata_from_Molgraph(data)
        print("data0 has been put on device: \n", data0.to(device))
        print("model has been put on device: \n", model.to(device))
        data0.batch = torch.zeros(data0.x.shape[0], dtype=torch.int64, device=device)
        explainer = Explainer(
            model=model,
